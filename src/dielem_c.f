...
c
     ...
     &           domain_type, domain_origin, num_front_nodes,
     &           front_order, out
      double precision :: feload(3,*), cf_tractions(*), rotate(3,3),
     &     dsf(mxndel,3,mxgp+1), jacobi(3,3,mxgp+1), cdispl(3,mxndel),
     &     qvals(*),
     &     iterm(8,7), coord(3,*), front_coords(3,*), ym_front_node,
     &     nu_front_node, crack_curvature(*),  jterm(9)
      logical :: cf_traction_flags(*), comput_j, comput_i,
     &            front_elem_flag, qp_node, face_loading, debug
c
c             locals
c
...
c
      integer :: nnode, iout
      double precision :: feload(3,*), eloads(3,*), cf_tractions(3),
     &                    rotate(3,3)
      logical :: debug
c
c             locals
...
c
      ...
      double precision :: dsf(mxndel,3,mxgp+1), jacobi(3,3,mxgp+1),
     &                    cdispl(3,mxndel),
     &                    eloads(3,mxndel), qvals(*), jterm(9)
      logical :: debug
c
c             locals
c
...
c             parameters
c
      integer ::  elemno, etype, nnode, nfnode, fnodes(mxndel),
     &            sfnodes(mxndel), domain_type, front_nodes(*),
     &            domain_origin, num_front_nodes, front_order, out
      double precision :: coord(3,*), front_coords(3,*),
     &     cf_tractions(*), cdispl(3,mxndel),
     &     eloads(3,mxndel), qvals(*), iterm(8,7), e_front, nu_front,
     &     crack_curvature(*)
...
c
     ...
     &           idumvec(1), ierr
      double precision ::
     &     fnode_coords(3,mxndel), xsi, eta, zeta,
     &     weight, dux, dvx, dwx, nx, ny, nz, cderiv_aux(27,3,7),
     &     termu, termv, termw, dum_vec(1), lg(mxgp+1), sf(mxndel),
     &     dsf(mxndel,3,mxgp+1), jacob(2,2), det_j, point_x, point_y,
...
c             parameters
c
      integer  :: nfnode, fnodes(*), sfnodes(*), elemno,
     &            front_nodes(*), num_front_nodes, domain_type,
     &            domain_origin, front_order, out
      double precision :: fnode_coords(3,*), ave_y_coord,
     &                    front_coords(3,*), crack_curvature(*)
c
c             locals
c
