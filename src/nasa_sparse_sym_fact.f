...
c
      implicit none
c
      integer, intent(in) :: neqns, xadj(*), adjncy(*)
      integer, intent(inout) :: xlnz(*), maxlnz, xnzsub(*), flag,
     &                          maxsub, nzsub(*)
c
c              locals
c      
      integer :: nzbeg, k, np1, knz, mrgk, mrkflg, node, jstrt,
     &           jstop, j, nabor, rchm, m, lmax, i, inz, kxsub, nzend  
      integer, allocatable :: perm(:), invp(:), rchlnk(:), 
     &                        mrglnk(:), marker(:)
c
c****************************************************************
c
...
c                             if  ( nzsub(jstrt) - i )  1000, 1100, 1300
                         if( nzsub(jstrt) - i < 0  ) go to 1000 
                         if( nzsub(jstrt) - i == 0 ) go to 1100 
                         if( nzsub(jstrt) - i > 0  ) go to 1300 
1000                    continue
                         go to 1300
 1100                    continue
                         xnzsub(k) = jstrt
