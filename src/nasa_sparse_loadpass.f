...
c  
      implicit none
c      
      integer, intent(in) :: neqns, xlnz(*), maxlnz, iloc(*), inum(*)
c      
      double precision, intent(in) :: coefs(*), diag(*)
      double precision, intent(inout) ::  rhs(*)
c      
      integer :: neq, i, j, istrt, istop, ii, isub, icont, mm, jcont, nn 
c      
      double precision :: sum, xx, rhsj
