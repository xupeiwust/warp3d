...
c     
      implicit none
c
      integer, intent(in) :: neq, ncoeff, itype
      integer :: kpt(*), kind(*), perm(*), xadj(*), adjncy(*) 
      double precision :: diag(*), rhs(*), coefs(*)
c
c              locals
c
      integer :: incof, isize_xdjncy, ncof, maxn, nterms, maxcon,
     &           nt, jmax
      integer, parameter :: nb = 2
      
      integer, allocatable :: jtemp(:), maxan(:), itemp(:), invp(:), 
     &                        deg(:), marker(:), qsize(:), qlink(:)
      double precision, allocatable :: xtemp(:), btemp(:), xdjncy(:)
c
c
      allocate( jtemp(neq+1), maxan(neq+1) )
c  
      call sparse_reorder2( neq, kpt, kind, jtemp, maxan, ncof, maxn )
c
      incof        = max(ncof,neq)
      isize_xdjncy = ((ncoeff * nb) + neq)
      allocate( itemp(incof), xtemp(ncof), xdjncy(isize_xdjncy) )
c
