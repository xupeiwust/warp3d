...
c
     ...
     &                            xdjncy, itemp, btemp )
      ...
      implicit none
c
      integer, intent(in) :: neqns, xadj(*), perm(*)                       
      integer, intent(inout) :: itemp(*), icoln(*), irow(*), adjncy(*)    
c                    
      double precision, intent(inout) :: a(*), d(*), btemp(*), b(*),
     &                                   xdjncy(*)   
c     
c              locals
c
      integer :: neq, ii, lcont, jq, is, iend, icont, ncont, i, j,
     &           jcont, jj, jmin, k, mycont
      integer, parameter :: ibig = 1000000
      double precision :: ak11
c
      neq = neqns
      do ii = 1, neq
        itemp(perm(ii)) = ii
...
c
      implicit none     
c      
      integer, intent(in) :: neq, irow(*), icoln(*), maxn, maxan(*) 
      integer, intent(inout) :: itemp(*), jtemp(*), nt, xadj(*),
     &                          adjncy(*), jmax
      double precision :: xtemp(*), a(*), xdjncy(*)
c
c              locals      
c 
      integer ::  ii, i, j, kcont, icont, ncont, iix, lcont 
c
      do ii = 1, neq
        jtemp(ii) = 0
      end do
...
c
      implicit none
c
      integer :: neq, ncof, maxn, irow(*), jtemp(*), maxan(*),
     &           icoln(*)             
c
c              locals
c
      integer :: i, ii, icont, j, iix, jmax
c        
...
c      
     ...
     1           marker(*), perm(*), qsize(*), xadj(*)
c
c              locals
c     
      integer :: delta, ehead, i, maxint, mdeg,
     1           mdlmt, mdnode, nextmd, num, tag
c
c***************************************************************
c
         maxint = 10000000
...
c
      implicit none
c      
      integer :: ehead, neqns, delta, mdeg, maxint, tag
      integer :: adjncy(*), dbakw(*), dforw(*), dhead(*),
     &              llist(*), marker(*), qsize(*), xadj(*)
c
c              locals
c     
      integer :: deg, deg0, elmnt, enode, fnode, i, iq2, istop,
     &           istrt, j, jstop, jstrt, link, mdeg0, mtag, nabor,
     &           node, q2head, qxhead
c
c***************************************************************
c
...
c                     if  ( enode )  400, 800, 500
                     if( enode < 0 ) go to 400
                     if( enode == 0 ) go to 800
...
c                             if  ( node )  1000, 2100, 1100
                             if( node < 0 ) go to 1000
                             if( node == 0 ) go to 2100
...
c
c
      subroutine sparse_reorder8( neqns, xadj, dhead, dforw, 
...
c
      implicit none
c      
      integer :: neqns, xadj(*), dhead(*), dbakw(*) , dforw(*),
     &           llist(*) , marker(*), qsize(*)
c
c              locals
c
...
c
c
      subroutine sparse_reorder9( neqns, perm, invp, qsize )
c
c***************************************************************
c
      implicit none
...
c              locals      
c
      integer :: father, nextf, node, nqsize, num, root
***************************************************************
c
         do node = 1, neqns
             nqsize = qsize(node)
